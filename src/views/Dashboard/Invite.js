...
export default function Invite() {
  ...
  async function updatePaymentStatus(ID) {
    ...
    const updateUserPaymentProcessing = await API.graphql({
      ...
      variables: { input: userDetailstoUpdate },
    })
      .then((data) => {
    ...
    if (location != "") {
      ...
      if (resultUpdateRefeerList == null) {
        ...
        if (resultUpdateRefeerList.id != null) {
          console.log(
            "Se pudo actualizar la lista de refiros",
            resultUpdateRefeerList,
          );
        } else {
          console.log(
            "Error actualizando lista de referidos",
            resultUpdateRefeerList,
          );
        }
      }
    ...
    if (location != "") {
      ...
      if (profileReferred == null) {
        ...
        if (profileRefeer.id != null) {
          console.log(
            "Usuario que compartio el link si existe, es",
            profileReferred,
          );
        } else {
          console.log(
            "Error con el usuario que compartio el link",
            profileReferred,
          );
        }
      }
  ...
  async function markingUserAsRefeer() {
    ...
    try {
      ...
      const result = await API.graphql(
        ...
        .then((result) => {
          console.log(
            "Resultado de la actualización del usuario actual",
            result,
          );
    ...
    try {
      ...
      if (profileRefeer.isCommercial) {
        ...
        if (profileRefeer.listUserReferredAsCommercial != null) {
          ...
          const objetoAuxiliarCommercial =
            profileRefeer.listUserReferredAsCommercial;
          objetoAuxiliarCommercial.push(profile.username);
          userDetailstoUpdate.listUserReferredAsCommercial =
            objetoAuxiliarCommercial;
        } else {
        ...
        if (profileRefeer.totalReferredCommercial != null) {
          //SI EL USUARIO ES COMERCIAL Y ES LA PRIMERA VEZ Q REFIERE COMERCIAL
          userDetailstoUpdate.totalReferredCommercial =
            profileRefeer.totalReferredCommercial + 1;
        } else {
      ...
      const result = await API.graphql(
        ...
        .then((result) => {
          console.log(
            "Resultado de la actualización del usuario que compartio link",
            result,
          );
          setIsLoadingRefeer(false);
          handleNext();
  ...
  async function getUserProfileRefeer(nam) {
    ...
    try {
      ...
      const result = await API.graphql(
        ...
        .then((result) => {
          console.log(
            "Resultado de la consulta del usuario que compartio link",
            result.data.searchUsers.items[0],
          );
  ...
  useEffect(() => {
    ...
    if (activeStep === 2) {
      console.log(
        "usuario que ha sido marcado como referido, actualizando lista de quien compartio el link...",
      );
      updateRefeers();
    }
    return () => {
        ...
        {profile && profile.isReferred ? (
          ...
          >
            <Text align={"center"} fontWeight={"bold"} fontSize={25}>
              You already has been referred
            </Text>
            <Text align={"center"} fontWeight={300}>
              We work hard to prevent errors, please if persist communicate to
              support@ohmiobroadcast.io
            </Text>
          </div>
        ) : (
          <>
            ...
            >
              <Text align={"center"} fontWeight={"bold"} fontSize={25}>
                Welcome to OHMIO Reefer System
              </Text>
              <Text align={"center"} fontWeight={300}>
                We are proccessing the Reefer of the user:
              </Text>
              {currentPath != "" ? (
                <Text
                  align={"center"}

...
function Manual() {
  ...
  async function getAllNotifications() {
    ...
    try {
      const getAllNotificationsData = await (
        await API.graphql(graphqlOperation(listNotifications))
      ).data;
  ...
  async function addNotification() {
    ...
    await DataStore.save(NotificationSignal)
      .then((data) => console.log("se añadio señal exitosa", data))
  ...
  async function requestModo() {
    const dataModo = await API.graphql(
      graphqlOperation(getModo, { id: `b8a7ec00-5a96-43af-899e-b76c1af0c365` }),
    );
    //console.log(dataModo)
    if (
      dataModo.data.getModo.automatic != null &&
  ...
  async function updateModoOperacion() {
    try {
      const updateModoData = await API.graphql(
        graphqlOperation(updateModo, {
          input: {
            id: `b8a7ec00-5a96-43af-899e-b76c1af0c365`,
            automatic: !automatic,
            _version: currentVersionMode,
          },
        }),
      );
      console.log(updateModoData);
      if (
        updateModoData.data.updateModo.automatic != null &&
                    ...
                    <Formik
                      ...
                      onSubmit={async (
                        ...
                        if (automatic == true) {
                          ...
                          const time12h = new Date().toLocaleTimeString(
                            "es-CO",
                            {
                              hour: "numeric",
                              minute: "numeric",
                  ...
                  >
                    ...
                    <Flex>
                      <Button
                        ...
                        h="45"
                        leftIcon={
                          <HiOutlineRefresh color="#FFFFFF" size={21} />
                        }
                        backgroundColor={"#ee5438"}
                        color={"white"}
                        onClick={() => requestModo()}
